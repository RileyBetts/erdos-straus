...
culminating in an unconditional exceptional-set bound stronger than Vaughan's 1970 theorem, and
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item a sequence of \textbf{analytic partial theorems and precisely stated negative results}
        on a covering-density reformulation of the conjecture (\Cref{sec:route1}), including an
        unconditional record improving on Vaughan's 1970 exceptional-set bound, together with
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item to produce, along the way, genuine unconditional mathematics that stands on its own
        merit regardless of whether the summit is reached — in particular a new exceptional-set
        bound for the full covering problem, stronger than the 1970 record;
...
\begin{description}[leftmargin=1.4em]
  \item[File-map layers (\file{README.md}).] \emph{Layer A} is the bare-Lean core —
        ...
        \file{NoVieta.lean} — compiling under Lean~$\ge 4.33.0$ with \emph{no imports at all},
...
unconditional exceptional-set bound $O(x^{1-c})$ \textbf{strictly stronger than Vaughan}. This
correction — explicitly flagged as such in the document, with the original \S4f retained
unedited as a record — is the origin of the E\_power theorem (\Cref{subsec:epower}).
...
\subsection{E\_power: an unconditional exceptional-set bound beating Vaughan}
\label{subsec:epower}

The programme's single strongest unconditional analytic result concerns the \emph{full}
...
\emph{superset} of the true ES exceptional set, so an upper bound here is automatically an upper
bound there.
...
\begin{theorem}[E\_power, \CLAIMED]
\label{thm:epower}
...
\end{theorem}
This is proved by combining Lemma~SM's second-moment input with Suen's inequality (giving
density $u(A)\le\exp(-(1-O(\eta))\mu(A))$) and a ``Lemma Transfer'' step: at
$A=\exp(c\sqrt{\log x})$, the hub modulus $M_T=\prod_{p\le T}p$ has size $x^{\alpha/\eta+o(1)}$
(with $\alpha=\kappa_{\mathrm{cov}}c^2$), and for $c$ small enough that $\alpha/\eta<1/2$ the
main term dominates the discrepancy over residues modulo $M_T$, giving the stated power saving.
Explicitly, $\delta=(1-O(\eta))\kappa_{\mathrm{cov}}c^2$ for the admissible range of $(c,\eta)$;
the numerical value of $\delta$ is not the content, effectivity and positivity are. The honest
comparison recorded throughout the archive: Theorem~\ref{thm:epower} \emph{strictly improves}
Vaughan's 1970 exponential-saving bound $\ll x\exp(-c(\log x)^{2/3})$ to a genuine power saving,
on the same object (all integers $n\le x$, not merely hard primes) — this is, on the programme's
own account, the single strongest unconditional new mathematical statement to come out of the
archive.
...
archive prevents a genuine, useful, unconditional record (Theorem~\ref{thm:epower}) from being
...
available. Second, a real unconditional theorem: Theorem~\ref{thm:epower} improves the 1970
Vaughan exceptional-set bound from an $\exp(-c(\log x)^{2/3})$ saving to a genuine power saving
$O(x^{1-\delta})$, via an elementary, fully written (if not machine-checked) second-moment
argument. Third, a substantial and unusually disciplined formalisation of the Bright--Loughran
...
reduction and of a non-trivial slice of the Bright--Loughran geometric theory; a new
unconditional exceptional-set bound stronger than the 55-year-old published record; a precisely
...
\texttt{erdos-\allowbreak straus-\allowbreak E-\allowbreak power.md} & Statement and full written proof of Theorem~\ref{thm:epower}
  (E\_power) and Lemma~SM. & \ref{subsec:epower} \\
