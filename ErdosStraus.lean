...
/-! ### Certificate ⇒ ClassRough (exact direction)

A completely multiplicative `m`-periodic sign with `χ(-1) = -1` is the
Layer-A stand-in for a real odd character mod `m`.  If every prime
factor of `N = p+4a²` coprime to `4a` lies in `ker χ`, then no divisor
of `N` can be `≡ −1 (mod 4a)`: such a divisor is automatically coprime
to `4a`, hence `χ` of it equals `1` by multiplicativity, but periodicity
gives `χ ≡ χ(-1) = -1`.  The converse (ClassRough ⇒ some certificate)
is a theorem at `a = 1, 2, 3` (`classRough_iff_chi4`,
`classRough_a2_iff_certificates`, `classRough_a3_iff_certificates`,
`classRough_123_iff_certificates`); for `a ≥ 4` it is an ω-gap
(`c4_certificates.py`). -/

/-- A real odd character mod `m`, as a completely multiplicative
    `m`-periodic function `Nat → ℤ` with `χ(m-1) = -1`. -/
structure OddRealChar (m : Nat) where
  χ : Nat → Int
  map_mul : ∀ a b, χ (a * b) = χ a * χ b
  map_add_mod : ∀ n, χ (n + m) = χ n
  odd : 0 < m → χ (m - 1) = -1

namespace OddRealChar

variable {m : Nat} (χ : OddRealChar m)

theorem map_mul_left (k n : Nat) : χ.χ (k * m + n) = χ.χ n := by
  induction k with
  | zero => simp
  | succ k ih =>
    have : (k + 1) * m + n = k * m + n + m := by
      rw [Nat.succ_mul]
      omega
    rw [this, χ.map_add_mod, ih]

theorem eval_mod (hm : 0 < m) (n : Nat) : χ.χ n = χ.χ (n % m) := by
  have hform : n = n / m * m + n % m := by
    rw [Nat.mul_comm (n / m), Nat.div_add_mod]
  have hmod : (n / m * m + n % m) % m = n % m := by
    rw [Nat.add_comm, Nat.add_mul_mod_self_right, Nat.mod_mod]
  rw [hform, hmod]
  exact χ.map_mul_left (n / m) (n % m)

theorem map_one (hm : 0 < m) : χ.χ 1 = 1 := by
  have h := χ.map_mul 1 1
  rw [Nat.mul_one] at h
  have hsq : χ.χ 1 * χ.χ 1 = χ.χ 1 := h.symm
  have hfac : χ.χ 1 * (χ.χ 1 - 1) = 0 := by
    rw [Int.mul_sub, Int.mul_one, hsq, Int.sub_self]
  rcases Int.mul_eq_zero.mp hfac with h0 | h1
  · have hodd := χ.odd hm
    have hmul1 : χ.χ (m - 1) = χ.χ (m - 1) * χ.χ 1 := by
      rw [← χ.map_mul, Nat.mul_one]
    rw [h0, Int.mul_zero] at hmul1
    have hz : χ.χ (m - 1) = 0 := hmul1
    rw [hz] at hodd
    exact absurd hodd (by decide)
  · exact Int.sub_eq_zero.mp h1

/-- An aligned divisor `q ≡ −1 (mod 4a)` is coprime to the modulus. -/
theorem gcd_eq_one_of_aligned {q a : Nat} (ha : 0 < a)
    (hmod : (q + 1) % (4 * a) = 0) :
    Nat.gcd q (4 * a) = 1 := by
  have hdvd : 4 * a ∣ q + 1 := Nat.dvd_of_mod_eq_zero hmod
  have hleft : Nat.gcd q (4 * a) ∣ q := Nat.gcd_dvd_left q (4 * a)
  have hright : Nat.gcd q (4 * a) ∣ 4 * a := Nat.gcd_dvd_right q (4 * a)
  have hqp1 : Nat.gcd q (4 * a) ∣ q + 1 := Nat.dvd_trans hright hdvd
  have hone : Nat.gcd q (4 * a) ∣ 1 := by
    simpa using Nat.dvd_sub hqp1 hleft
  exact Nat.eq_one_of_dvd_one hone

theorem mod_pred_of_succ_mod_zero {q M : Nat} (hM : 0 < M)
    (h : (q + 1) % M = 0) : q % M = M - 1 := by
  have hdvd : M ∣ q + 1 := Nat.dvd_of_mod_eq_zero h
  obtain ⟨k, hk⟩ := hdvd
  have hkpos : 0 < k := by
    ...
    · subst h0
      simp at hk
    · exact h0
  have hprod : q + 1 = k * M := by
    rw [hk, Nat.mul_comm]
  have hq : q = (k - 1) * M + (M - 1) := by
    have : k * M = (k - 1) * M + M := by
      conv => lhs; rw [show k = k - 1 + 1 from (by omega)]
      rw [Nat.add_mul, Nat.one_mul]
    omega
  have hlt : M - 1 < M := Nat.sub_lt hM (by omega)
  rw [hq, Nat.add_comm, Nat.add_mul_mod_self_right, Nat.mod_eq_of_lt hlt]

/-- Complete multiplicativity plus a prime-factor certificate yields
    `χ(q) = 1` for every coprime divisor of `N`. -/
theorem chi_eq_one_of_dvd (hm : 0 < m) (N q : Nat)
    (hcert : ∀ p, IsPrime p → p ∣ N → Nat.gcd p m = 1 → χ.χ p = 1)
    (hN : q ∣ N) (hcop : Nat.gcd q m = 1) :
    χ.χ q = 1 := by
  induction q using Nat.strongRecOn with
  | ind q ih =>
    by_cases h0 : q = 0
    · subst h0
      have hm1 : m = 1 := by simpa [Nat.gcd_zero_left] using hcop
      have hχ0 : χ.χ 0 = χ.χ 1 := by
        have := (χ.map_add_mod 0).symm
        simpa [hm1] using this
      exact hχ0.trans (χ.map_one hm)
    · by_cases h1 : q = 1
      · subst h1
        exact χ.map_one hm
      · have hq2 : 2 ≤ q := by omega
        obtain ⟨p, hp, hpdvd⟩ := exists_prime_factor q hq2
        have hpN : p ∣ N := Nat.dvd_trans hpdvd hN
        have hpgcd : Nat.gcd p m ∣ Nat.gcd q m :=
          Nat.dvd_gcd (Nat.dvd_trans (Nat.gcd_dvd_left p m) hpdvd)
            (Nat.gcd_dvd_right p m)
        have hpcop : Nat.gcd p m = 1 :=
          Nat.eq_one_of_dvd_one (by simpa [hcop] using hpgcd)
        have hpχ : χ.χ p = 1 := hcert p hp hpN hpcop
        have hpeq : p * (q / p) = q := Nat.mul_div_cancel' hpdvd
        have hp2 : 2 ≤ p := hp.1
        have hplt : q / p < q := by
          have hle : q / p * 2 ≤ q / p * p := Nat.mul_le_mul_left _ hp2
          have : q / p * p = q := by rw [Nat.mul_comm, hpeq]
          omega
        have hquot_q : q / p ∣ q := ⟨p, by rw [Nat.mul_comm, hpeq]⟩
        have hquot_dvd : q / p ∣ N := Nat.dvd_trans hquot_q hN
        have hquot_cop : Nat.gcd (q / p) m = 1 := by
          have hd : Nat.gcd (q / p) m ∣ q :=
            Nat.dvd_trans (Nat.gcd_dvd_left _ _) hquot_q
          have : Nat.gcd (q / p) m ∣ Nat.gcd q m :=
            Nat.dvd_gcd hd (Nat.gcd_dvd_right _ _)
          exact Nat.eq_one_of_dvd_one (by simpa [hcop] using this)
        have hquot : χ.χ (q / p) = 1 := ih (q / p) hplt hquot_dvd hquot_cop
        have hmul := χ.map_mul p (q / p)
        rw [hpeq, hpχ, hquot, Int.one_mul] at hmul
        exact hmul

end OddRealChar

/-- **Certificate ⇒ ClassRough** (exact).  If a real odd character mod `4a`
    certifies every coprime prime factor of `p+4a²`, then `p+4a²` has no
    aligned divisor.  Welds the character strand (`survivor_legendre_three`
    in Layer B) to the roughness core: both are odd-character constraints,
    this one on the *factors* of the shift. -/
theorem classRough_of_certifies {p a : Nat} (ha : 0 < a)
    (χ : OddRealChar (4 * a))
    (hcert : ∀ q, IsPrime q → q ∣ p + 4 * (a * a) →
      Nat.gcd q (4 * a) = 1 → χ.χ q = 1) :
    ClassRough p a := by
  intro q hq3 hmod hdvd
  have hm : 0 < 4 * a := by omega
  have hcop := OddRealChar.gcd_eq_one_of_aligned ha hmod
  have hχq : χ.χ q = 1 :=
    χ.chi_eq_one_of_dvd hm (p + 4 * (a * a)) q hcert hdvd hcop
  have hrem : q % (4 * a) = 4 * a - 1 :=
    OddRealChar.mod_pred_of_succ_mod_zero hm hmod
  have hχrem : χ.χ q = χ.χ (4 * a - 1) := by
    rw [χ.eval_mod hm, hrem]
  have hodd : χ.χ (4 * a - 1) = -1 := χ.odd hm
  rw [hχrem, hodd] at hχq
  exact absurd hχq (by decide)

/-- The unique real odd character modulo 4.  Inhabitant of `OddRealChar`
    at `a = 1`: ClassRough of `p+4` is 3-mod-4-freeness of its odd prime
    factors. -/
def chi4 (n : Nat) : Int :=
  if n % 2 = 0 then 0
...
theorem chi4_mul (a b : Nat) : chi4 (a * b) = chi4 a * chi4 b := by
  rcases Nat.mod_two_eq_zero_or_one a with ha | ha
  · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, ha]
    simp [chi4, ha, hab]
  · rcases Nat.mod_two_eq_zero_or_one b with hb | hb
    · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, hb]
      simp [chi4, hb, hab]
    · have ha4 : a % 4 = 1 ∨ a % 4 = 3 := by
        have hlt : a % 4 < 4 := Nat.mod_lt a (by decide)
        have h24 : (a % 4) % 2 = a % 2 :=
          Nat.mod_mod_of_dvd a (show 2 ∣ 4 by decide)
        rw [ha] at h24
        omega
      have hb4 : b % 4 = 1 ∨ b % 4 = 3 := by
        have hlt : b % 4 < 4 := Nat.mod_lt b (by decide)
        have h24 : (b % 4) % 2 = b % 2 :=
          Nat.mod_mod_of_dvd b (show 2 ∣ 4 by decide)
        rw [hb] at h24
        omega
      have hab : (a * b) % 4 = ((a % 4) * (b % 4)) % 4 := Nat.mul_mod a b 4
      rcases ha4 with ha1 | ha3 <;> rcases hb4 with hb1 | hb3
      · have : (a * b) % 4 = 1 := by rw [hab, ha1, hb1]
        have hab2 : (a * b) % 2 = 1 := by
          have h24 : ((a * b) % 4) % 2 = (a * b) % 2 :=
            Nat.mod_mod_of_dvd (a * b) (show 2 ∣ 4 by decide)
          omega
        simp [chi4, ha, hb, hab2, ha1, hb1, this]
      · have : (a * b) % 4 = 3 := by rw [hab, ha1, hb3]
        have hab2 : (a * b) % 2 = 1 := by
          have h24 : ((a * b) % 4) % 2 = (a * b) % 2 :=
            Nat.mod_mod_of_dvd (a * b) (show 2 ∣ 4 by decide)
          omega
        simp [chi4, ha, hb, hab2, ha1, hb3, this]
      · have : (a * b) % 4 = 3 := by rw [hab, ha3, hb1]
        have hab2 : (a * b) % 2 = 1 := by
          have h24 : ((a * b) % 4) % 2 = (a * b) % 2 :=
            Nat.mod_mod_of_dvd (a * b) (show 2 ∣ 4 by decide)
          omega
        simp [chi4, ha, hb, hab2, ha3, hb1, this]
      · have : (a * b) % 4 = 1 := by rw [hab, ha3, hb3]
        have hab2 : (a * b) % 2 = 1 := by
          have h24 : ((a * b) % 4) % 2 = (a * b) % 2 :=
            Nat.mod_mod_of_dvd (a * b) (show 2 ∣ 4 by decide)
          omega
        simp [chi4, ha, hb, hab2, ha3, hb3, this]

theorem chi4_add_four (n : Nat) : chi4 (n + 4) = chi4 n := by
  simp [chi4, Nat.add_mod]

def oddRealChar_four : OddRealChar 4 where
  χ := chi4
  map_mul := chi4_mul
  map_add_mod := chi4_add_four
  odd := fun _ => by decide

/-- `a = 1` case of `classRough_of_certifies`: if every odd prime factor
    of `p+4` is `1 (mod 4)`, then `ClassRough p 1`. -/
theorem classRough_of_chi4 {p : Nat}
    (hcert : ∀ q, IsPrime q → q ∣ p + 4 → Nat.gcd q 4 = 1 → chi4 q = 1) :
    ClassRough p 1 :=
  classRough_of_certifies (by decide : 0 < 1) oddRealChar_four fun q hq hN hg =>
    hcert q hq (by simpa using hN) (by simpa using hg)

theorem gcd_four_eq_one_odd {q : Nat} (h : Nat.gcd q 4 = 1) : q % 2 = 1 := by
  have h2 : ¬ 2 ∣ q := by
    intro hd
    have : 2 ∣ Nat.gcd q 4 := Nat.dvd_gcd hd (by decide : 2 ∣ 4)
    have : 2 ∣ 1 := by simpa [h] using this
    omega
  rcases Nat.mod_two_eq_zero_or_one q with he | ho
  · exact False.elim (h2 (Nat.dvd_of_mod_eq_zero he))
  · exact ho

/-- Converse at `a = 1`: ClassRough is exactly the χ₄-certificate.
    Aligned residues are `≡ 3 (mod 4)`, so there is no composite extra. -/
theorem chi4_of_classRough {p : Nat} (h : ClassRough p 1)
    {q : Nat} (hq : IsPrime q) (hdvd : q ∣ p + 4) (hcop : Nat.gcd q 4 = 1) :
    chi4 q = 1 := by
  have hodd := gcd_four_eq_one_odd hcop
  have hne2 : q ≠ 2 := by
    intro h2; subst h2; cases hodd
  rcases IsPrime.ne_two_mod4 hq hne2 with h1 | h3
  · simp [chi4, hodd, h1]
  · have hq3 : 3 ≤ q := by
      have : 2 ≤ q := hq.1
      omega
    have : ¬ q ∣ p + 4 := h q hq3 (by omega)
    exact absurd hdvd this

theorem classRough_iff_chi4 {p : Nat} :
    ClassRough p 1 ↔
      ∀ q, IsPrime q → q ∣ p + 4 → Nat.gcd q 4 = 1 → chi4 q = 1 :=
  ⟨fun h _ hq hdvd hcop => chi4_of_classRough h hq hdvd hcop, classRough_of_chi4⟩

/-- Kronecker χ₈ = (2/·): `+1` on `1,7 (mod 8)`, `-1` on `3,5 (mod 8)`. -/
def chi8 (n : Nat) : Int :=
  if n % 2 = 0 then 0
  else if n % 8 = 1 ∨ n % 8 = 7 then 1
  else -1

theorem odd_mod8 {n : Nat} (h : n % 2 = 1) :
    n % 8 = 1 ∨ n % 8 = 3 ∨ n % 8 = 5 ∨ n % 8 = 7 := by
  have hlt : n % 8 < 8 := Nat.mod_lt n (by decide)
  have h28 : (n % 8) % 2 = n % 2 :=
    Nat.mod_mod_of_dvd n (show 2 ∣ 8 by decide)
  rw [h] at h28
  omega

theorem chi8_mul (a b : Nat) : chi8 (a * b) = chi8 a * chi8 b := by
  rcases Nat.mod_two_eq_zero_or_one a with ha | ha
  · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, ha]
    simp [chi8, ha, hab]
  · rcases Nat.mod_two_eq_zero_or_one b with hb | hb
    · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, hb]
      simp [chi8, hb, hab]
    · have hab2 : (a * b) % 2 = 1 := by simp [Nat.mul_mod, ha, hb]
      have hab : (a * b) % 8 = ((a % 8) * (b % 8)) % 8 := Nat.mul_mod a b 8
      rcases odd_mod8 ha with ha1 | ha3 | ha5 | ha7
        <;> rcases odd_mod8 hb with hb1 | hb3 | hb5 | hb7
      · have : (a * b) % 8 = 1 := by rw [hab, ha1, hb1]
        simp [chi8, ha, hb, hab2, ha1, hb1, this]
      · have : (a * b) % 8 = 3 := by rw [hab, ha1, hb3]
        simp [chi8, ha, hb, hab2, ha1, hb3, this]
      · have : (a * b) % 8 = 5 := by rw [hab, ha1, hb5]
        simp [chi8, ha, hb, hab2, ha1, hb5, this]
      · have : (a * b) % 8 = 7 := by rw [hab, ha1, hb7]
        simp [chi8, ha, hb, hab2, ha1, hb7, this]
      · have : (a * b) % 8 = 3 := by rw [hab, ha3, hb1]
        simp [chi8, ha, hb, hab2, ha3, hb1, this]
      · have : (a * b) % 8 = 1 := by rw [hab, ha3, hb3]
        simp [chi8, ha, hb, hab2, ha3, hb3, this]
      · have : (a * b) % 8 = 7 := by rw [hab, ha3, hb5]
        simp [chi8, ha, hb, hab2, ha3, hb5, this]
      · have : (a * b) % 8 = 5 := by rw [hab, ha3, hb7]
        simp [chi8, ha, hb, hab2, ha3, hb7, this]
      · have : (a * b) % 8 = 5 := by rw [hab, ha5, hb1]
        simp [chi8, ha, hb, hab2, ha5, hb1, this]
      · have : (a * b) % 8 = 7 := by rw [hab, ha5, hb3]
        simp [chi8, ha, hb, hab2, ha5, hb3, this]
      · have : (a * b) % 8 = 1 := by rw [hab, ha5, hb5]
        simp [chi8, ha, hb, hab2, ha5, hb5, this]
      · have : (a * b) % 8 = 3 := by rw [hab, ha5, hb7]
        simp [chi8, ha, hb, hab2, ha5, hb7, this]
      · have : (a * b) % 8 = 7 := by rw [hab, ha7, hb1]
        simp [chi8, ha, hb, hab2, ha7, hb1, this]
      · have : (a * b) % 8 = 5 := by rw [hab, ha7, hb3]
        simp [chi8, ha, hb, hab2, ha7, hb3, this]
      · have : (a * b) % 8 = 3 := by rw [hab, ha7, hb5]
        simp [chi8, ha, hb, hab2, ha7, hb5, this]
      · have : (a * b) % 8 = 1 := by rw [hab, ha7, hb7]
        simp [chi8, ha, hb, hab2, ha7, hb7, this]

theorem chi8_add_eight (n : Nat) : chi8 (n + 8) = chi8 n := by
  simp [chi8, Nat.add_mod]

theorem odd_of_mod8 {n r : Nat} (hr : r % 2 = 1) (h : n % 8 = r) : n % 2 = 1 := by
  have h28 : (n % 8) % 2 = n % 2 := Nat.mod_mod_of_dvd n (show 2 ∣ 8 by decide)
  omega

theorem chi8_one {n : Nat} (h : n % 8 = 1) : chi8 n = 1 := by
  have hodd := odd_of_mod8 (by decide) h
  simp [chi8, hodd, h]

theorem chi8_seven {n : Nat} (h : n % 8 = 7) : chi8 n = 1 := by
  have hodd := odd_of_mod8 (by decide) h
  simp [chi8, hodd, h]

theorem chi8_three {n : Nat} (h : n % 8 = 3) : chi8 n = -1 := by
  have hodd := odd_of_mod8 (by decide) h
  have hn : ¬ (n % 8 = 1 ∨ n % 8 = 7) := by omega
  simp [chi8, hodd, hn]

theorem chi8_five {n : Nat} (h : n % 8 = 5) : chi8 n = -1 := by
  have hodd := odd_of_mod8 (by decide) h
  have hn : ¬ (n % 8 = 1 ∨ n % 8 = 7) := by omega
  simp [chi8, hodd, hn]

theorem chi4_of_mod8_one {n : Nat} (h : n % 8 = 1) : chi4 n = 1 := by
  have hodd := odd_of_mod8 (by decide) h
  have h4 : n % 4 = 1 := by omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod8_three {n : Nat} (h : n % 8 = 3) : chi4 n = -1 := by
  have hodd := odd_of_mod8 (by decide) h
  have h4 : n % 4 = 3 := by omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod8_five {n : Nat} (h : n % 8 = 5) : chi4 n = 1 := by
  have hodd := odd_of_mod8 (by decide) h
  have h4 : n % 4 = 1 := by omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod8_seven {n : Nat} (h : n % 8 = 7) : chi4 n = -1 := by
  have hodd := odd_of_mod8 (by decide) h
  have h4 : n % 4 = 3 := by omega
  simp [chi4, hodd, h4]

theorem chi4_add_eight (n : Nat) : chi4 (n + 8) = chi4 n := by
  have h1 := chi4_add_four n
  have h2 := chi4_add_four (n + 4)
  have : n + 8 = n + 4 + 4 := by omega
  rw [this, h2, h1]

def oddRealChar_eight_chi4 : OddRealChar 8 where
  χ := chi4
  map_mul := chi4_mul
  map_add_mod := chi4_add_eight
  odd := fun _ => by decide

def chi4chi8 (n : Nat) : Int := chi4 n * chi8 n

theorem chi4chi8_mul (a b : Nat) :
    chi4chi8 (a * b) = chi4chi8 a * chi4chi8 b := by
  unfold chi4chi8
  rw [chi4_mul, chi8_mul]
  calc
    chi4 a * chi4 b * (chi8 a * chi8 b)
        = chi4 a * (chi4 b * (chi8 a * chi8 b)) := by rw [Int.mul_assoc]
    _ = chi4 a * (chi8 a * (chi4 b * chi8 b)) := by
        rw [← Int.mul_assoc (chi4 b), Int.mul_comm (chi4 b), Int.mul_assoc]
    _ = (chi4 a * chi8 a) * (chi4 b * chi8 b) := by rw [← Int.mul_assoc]

theorem chi4chi8_add_eight (n : Nat) : chi4chi8 (n + 8) = chi4chi8 n := by
  simp [chi4chi8, chi4_add_eight, chi8_add_eight]

def oddRealChar_eight_chi4chi8 : OddRealChar 8 where
  χ := chi4chi8
  map_mul := chi4chi8_mul
  map_add_mod := chi4chi8_add_eight
  odd := fun _ => by decide

theorem gcd_eight_eq_one_odd {q : Nat} (h : Nat.gcd q 8 = 1) : q % 2 = 1 := by
  have h2 : ¬ 2 ∣ q := by
    intro hd
    have : 2 ∣ Nat.gcd q 8 := Nat.dvd_gcd hd (by decide : 2 ∣ 8)
    have : 2 ∣ 1 := by simpa [h] using this
    omega
  rcases Nat.mod_two_eq_zero_or_one q with he | ho
  · exact False.elim (h2 (Nat.dvd_of_mod_eq_zero he))
...
theorem mul_dvd_of_distinct_primes {r s N : Nat}
    (hr : IsPrime r) (hs : IsPrime s) (hne : r ≠ s)
    (hrN : r ∣ N) (hsN : s ∣ N) : r * s ∣ N := by
  obtain ⟨k, hk⟩ := hrN
  have hs' : s ∣ r * k := by rw [← hk]; exact hsN
  rcases hs.dvd_or_dvd hs' with hsr | hsk
  · have h1or : s = 1 ∨ s = r := hr.2 s hsr
    rcases h1or with h1 | heq
    · have : 2 ≤ s := hs.1; omega
    · exact False.elim (hne heq.symm)
  · obtain ⟨m, hm⟩ := hsk
    refine ⟨m, ?_⟩
    rw [hk, hm, Nat.mul_assoc]

/-- The two odd real characters mod 8. `χ₄` kers `{1,5}`; `χ₄χ₈` kers `{1,3}`.
    A `7 (mod 8)` divisor is either a `7`-prime or a `3×5` product, so
    ClassRough at `a = 2` is exactly the union of the two certificates. -/
theorem classRough_of_a2_chi4 {p : Nat}
    (hcert : ∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4 q = 1) :
    ClassRough p 2 :=
  classRough_of_certifies (by decide : 0 < 2) oddRealChar_eight_chi4 fun q hq hN hg => by
    have : p + 4 * (2 * 2) = p + 16 := by simp
    exact hcert q hq (by simpa [this] using hN) (by simpa using hg)

theorem classRough_of_a2_chi4chi8 {p : Nat}
    (hcert : ∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4chi8 q = 1) :
    ClassRough p 2 :=
  classRough_of_certifies (by decide : 0 < 2) oddRealChar_eight_chi4chi8
    fun q hq hN hg => by
    have : p + 4 * (2 * 2) = p + 16 := by simp
    exact hcert q hq (by simpa [this] using hN) (by simpa using hg)

theorem classRough_a2_of_certificates {p : Nat}
    (h : (∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4 q = 1) ∨
         (∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4chi8 q = 1)) :
    ClassRough p 2 := by
  rcases h with h | h
  · exact classRough_of_a2_chi4 h
  · exact classRough_of_a2_chi4chi8 h

theorem chi4_eq_one_mod8 {q : Nat} (h : q % 2 = 1) :
    chi4 q = 1 ↔ q % 8 = 1 ∨ q % 8 = 5 := by
  have h13 : q % 4 = 1 ∨ q % 4 = 3 := by
    have hlt : q % 4 < 4 := Nat.mod_lt q (by decide)
    have h24 : (q % 4) % 2 = q % 2 :=
      Nat.mod_mod_of_dvd q (show 2 ∣ 4 by decide)
    omega
  have h48 : q % 8 % 4 = q % 4 := Nat.mod_mod_of_dvd q (show 4 ∣ 8 by decide)
  constructor
  · intro hχ
    have : q % 4 = 1 := by
      simp [chi4, h] at hχ
      ...
      · exact h1
      · simp [h3] at hχ
    rcases odd_mod8 h with h1 | h3 | h5 | h7
    · exact Or.inl h1
    · omega
    · exact Or.inr h5
    · omega
  · intro hq
    rcases hq with h1 | h5
    · have : q % 4 = 1 := by omega
      simp [chi4, h, this]
    · have : q % 4 = 1 := by omega
      simp [chi4, h, this]

theorem chi4chi8_eq_one_mod8 {q : Nat} (h : q % 2 = 1) :
    chi4chi8 q = 1 ↔ q % 8 = 1 ∨ q % 8 = 3 := by
  rcases odd_mod8 h with h1 | h3 | h5 | h7
  · constructor
    · intro; exact Or.inl h1
    · intro
      simp [chi4chi8, chi4_of_mod8_one h1, chi8_one h1]
  · constructor
    · intro; exact Or.inr h3
    · intro
      simp [chi4chi8, chi4_of_mod8_three h3, chi8_three h3]
  · constructor
    · intro hχ
      simp [chi4chi8, chi4_of_mod8_five h5, chi8_five h5] at hχ
    · intro hq
      rcases hq with h1' | h3' <;> omega
  · constructor
    · intro hχ
      simp [chi4chi8, chi4_of_mod8_seven h7, chi8_seven h7] at hχ
    · intro hq
      rcases hq with h1' | h3' <;> omega

theorem classRough_a2_iff_certificates {p : Nat} :
    ClassRough p 2 ↔
      (∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4 q = 1) ∨
      (∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4chi8 q = 1) := by
  constructor
  · intro h
    by_cases h4 : ∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4 q = 1
    · exact Or.inl h4
    · right
      intro q hq hdvd hcop
      have hodd := gcd_eight_eq_one_odd hcop
      have hne2 : q ≠ 2 := by intro h2; subst h2; cases hodd
      by_cases hχ : chi4chi8 q = 1
      · exact hχ
      · -- q not in ker χ₄χ₈, so q ≡ 5 or 7 (mod 8). Produce an aligned divisor.
        have hq8 := odd_mod8 hodd
        have hn13 : ¬ (q % 8 = 1 ∨ q % 8 = 3) := by
          intro h13
          have : chi4chi8 q = 1 := (chi4chi8_eq_one_mod8 hodd).mpr h13
          exact hχ this
        have h57 : q % 8 = 5 ∨ q % 8 = 7 := by
          rcases hq8 with h1 | h3 | h5 | h7
          · exact False.elim (hn13 (Or.inl h1))
          · exact False.elim (hn13 (Or.inr h3))
          · exact Or.inl h5
          · exact Or.inr h7
        rcases h57 with h5 | h7
        · -- q ≡ 5: χ₄ holds on q. ClassRough + not-all-χ₄ ⇒ some r ≡ 3 or 7.
          have hnot4 := h4
          obtain ⟨r, hr⟩ := Classical.not_forall.mp hnot4
          obtain ⟨hrp, hr⟩ := Classical.not_imp.mp hr
          obtain ⟨hrd, hr⟩ := Classical.not_imp.mp hr
          obtain ⟨hrc, hrχ⟩ := Classical.not_imp.mp hr
          have hrodd := gcd_eight_eq_one_odd hrc
          have hrne2 : r ≠ 2 := by intro h2; subst h2; cases hrodd
          have hn15 : ¬ (r % 8 = 1 ∨ r % 8 = 5) := by
            intro h15
            have : chi4 r = 1 := (chi4_eq_one_mod8 hrodd).mpr h15
            exact hrχ this
          have hr37 : r % 8 = 3 ∨ r % 8 = 7 := by
            rcases odd_mod8 hrodd with h1 | h3 | h5' | h7
            · exact False.elim (hn15 (Or.inl h1))
            · exact Or.inl h3
            · exact False.elim (hn15 (Or.inr h5'))
            · exact Or.inr h7
          rcases hr37 with hr3 | hr7
          · -- r ≡ 3, q ≡ 5 ⇒ r*q ≡ 15 ≡ 7 (mod 8)
            have hne : r ≠ q := by
              intro heq; subst heq; omega
            have hprod : r * q ∣ p + 16 :=
              mul_dvd_of_distinct_primes hrp hq hne hrd hdvd
            have hmod : (r * q + 1) % 8 = 0 := by
              have : (r * q) % 8 = 7 := by
                have := Nat.mul_mod r q 8
                rw [hr3, h5] at this
                exact this
              omega
            have h3s : 3 ≤ r * q := by
              have hr2 : 2 ≤ r := hrp.1
              have hq2 : 2 ≤ q := hq.1
              have : 4 ≤ r * q := Nat.mul_le_mul hr2 hq2
              omega
            exact absurd hprod (h (r * q) (by omega) hmod)
          · have h3s : 3 ≤ r := by have := hrp.1; omega
            exact absurd hrd (h r h3s (by omega))
        · have h3s : 3 ≤ q := by have := hq.1; omega
          exact absurd hdvd (h q h3s (by omega))
  · exact classRough_a2_of_certificates

/-- Kronecker (3/·) as a character mod 12: `+1` on `1,11`, `-1` on `5,7`. -/
def chi3mod12 (n : Nat) : Int :=
  if n % 2 = 0 ∨ n % 3 = 0 then 0
  else if n % 12 = 1 ∨ n % 12 = 11 then 1
  else -1

theorem gcd_twelve_eq_one {q : Nat} (h : Nat.gcd q 12 = 1) :
    q % 2 = 1 ∧ q % 3 ≠ 0 := by
  have h2 : ¬ 2 ∣ q := by
    intro hd
    have : 2 ∣ Nat.gcd q 12 := Nat.dvd_gcd hd (by decide : 2 ∣ 12)
    have : 2 ∣ 1 := by simpa [h] using this
    omega
  have h3 : ¬ 3 ∣ q := by
    intro hd
    have : 3 ∣ Nat.gcd q 12 := Nat.dvd_gcd hd (by decide : 3 ∣ 12)
    have : 3 ∣ 1 := by simpa [h] using this
    omega
  constructor
  · rcases Nat.mod_two_eq_zero_or_one q with he | ho
    · exact False.elim (h2 (Nat.dvd_of_mod_eq_zero he))
    · exact ho
  · intro h0
    exact h3 (Nat.dvd_of_mod_eq_zero h0)

theorem coprime12_mod {n : Nat} (h : Nat.gcd n 12 = 1) :
    n % 12 = 1 ∨ n % 12 = 5 ∨ n % 12 = 7 ∨ n % 12 = 11 := by
  have ⟨hodd, h3⟩ := gcd_twelve_eq_one h
  have hlt : n % 12 < 12 := Nat.mod_lt n (by decide)
  have h212 : (n % 12) % 2 = n % 2 :=
    Nat.mod_mod_of_dvd n (show 2 ∣ 12 by decide)
  have h312 : (n % 12) % 3 = n % 3 :=
    Nat.mod_mod_of_dvd n (show 3 ∣ 12 by decide)
  rw [hodd] at h212
  omega

theorem mod_mod_two_of_twelve (n : Nat) : (n % 12) % 2 = n % 2 :=
  Nat.mod_mod_of_dvd n (show 2 ∣ 12 by decide)

theorem mod_mod_three_of_twelve (n : Nat) : (n % 12) % 3 = n % 3 :=
  Nat.mod_mod_of_dvd n (show 3 ∣ 12 by decide)

theorem mod_mod_four_of_twelve (n : Nat) : (n % 12) % 4 = n % 4 :=
  Nat.mod_mod_of_dvd n (show 4 ∣ 12 by decide)

theorem mul_mod3_ne_zero {a b : Nat} (ha : a % 3 ≠ 0) (hb : b % 3 ≠ 0) :
    (a * b) % 3 ≠ 0 := by
  have hla : a % 3 < 3 := Nat.mod_lt a (by decide)
  have hlb : b % 3 < 3 := Nat.mod_lt b (by decide)
  have ha12 : a % 3 = 1 ∨ a % 3 = 2 := by omega
  have hb12 : b % 3 = 1 ∨ b % 3 = 2 := by omega
  have hab : (a * b) % 3 = ((a % 3) * (b % 3)) % 3 := Nat.mul_mod a b 3
  rcases ha12 with ha1 | ha2 <;> rcases hb12 with hb1 | hb2
  · simp [hab, ha1, hb1]
  · simp [hab, ha1, hb2]
  · simp [hab, ha2, hb1]
  · simp [hab, ha2, hb2]

theorem chi3mod12_one {n : Nat} (h : n % 12 = 1) : chi3mod12 n = 1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have hn3 : n % 3 = 1 := by have := mod_mod_three_of_twelve n; omega
  have hz : ¬ (n % 2 = 0 ∨ n % 3 = 0) := by omega
  simp [chi3mod12, hz, h]

theorem chi3mod12_eleven {n : Nat} (h : n % 12 = 11) : chi3mod12 n = 1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have hn3 : n % 3 = 2 := by have := mod_mod_three_of_twelve n; omega
  have hz : ¬ (n % 2 = 0 ∨ n % 3 = 0) := by omega
  simp [chi3mod12, hz, h]

theorem chi3mod12_five {n : Nat} (h : n % 12 = 5) : chi3mod12 n = -1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have hn3 : n % 3 = 2 := by have := mod_mod_three_of_twelve n; omega
  have hz : ¬ (n % 2 = 0 ∨ n % 3 = 0) := by omega
  have hn : ¬ (n % 12 = 1 ∨ n % 12 = 11) := by omega
  simp [chi3mod12, hz, hn]

theorem chi3mod12_seven {n : Nat} (h : n % 12 = 7) : chi3mod12 n = -1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have hn3 : n % 3 = 1 := by have := mod_mod_three_of_twelve n; omega
  have hz : ¬ (n % 2 = 0 ∨ n % 3 = 0) := by omega
  have hn : ¬ (n % 12 = 1 ∨ n % 12 = 11) := by omega
...
theorem chi4_of_mod12_one {n : Nat} (h : n % 12 = 1) : chi4 n = 1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have h4 : n % 4 = 1 := by have := mod_mod_four_of_twelve n; omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod12_five {n : Nat} (h : n % 12 = 5) : chi4 n = 1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have h4 : n % 4 = 1 := by have := mod_mod_four_of_twelve n; omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod12_seven {n : Nat} (h : n % 12 = 7) : chi4 n = -1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have h4 : n % 4 = 3 := by have := mod_mod_four_of_twelve n; omega
  simp [chi4, hodd, h4]

theorem chi4_of_mod12_eleven {n : Nat} (h : n % 12 = 11) : chi4 n = -1 := by
  have hodd : n % 2 = 1 := by have := mod_mod_two_of_twelve n; omega
  have h4 : n % 4 = 3 := by have := mod_mod_four_of_twelve n; omega
  simp [chi4, hodd, h4]

theorem chi3mod12_mul (a b : Nat) :
    chi3mod12 (a * b) = chi3mod12 a * chi3mod12 b := by
  by_cases ha2 : a % 2 = 0
  · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, ha2]
    simp [chi3mod12, ha2, hab]
  · by_cases hb2 : b % 2 = 0
    · have hab : (a * b) % 2 = 0 := by simp [Nat.mul_mod, hb2]
      simp [chi3mod12, hb2, hab]
    · by_cases ha3 : a % 3 = 0
      · have hab : (a * b) % 3 = 0 := by simp [Nat.mul_mod, ha3]
        have ha0 : a % 2 = 0 ∨ a % 3 = 0 := Or.inr ha3
        have hab0 : (a * b) % 2 = 0 ∨ (a * b) % 3 = 0 := Or.inr hab
        simp [chi3mod12, ha0, hab0]
      · by_cases hb3 : b % 3 = 0
        · have hab : (a * b) % 3 = 0 := by simp [Nat.mul_mod, hb3]
          have hb0 : b % 2 = 0 ∨ b % 3 = 0 := Or.inr hb3
          have hab0 : (a * b) % 2 = 0 ∨ (a * b) % 3 = 0 := Or.inr hab
          simp [chi3mod12, hb0, hab0]
        · have ha2' : a % 2 = 1 := by
            rcases Nat.mod_two_eq_zero_or_one a with h | h
            · exact False.elim (ha2 h)
            · exact h
          have hb2' : b % 2 = 1 := by
            rcases Nat.mod_two_eq_zero_or_one b with h | h
            · exact False.elim (hb2 h)
            · exact h
          have hab2 : (a * b) % 2 = 1 := by simp [Nat.mul_mod, ha2', hb2']
          have hab3 : (a * b) % 3 ≠ 0 := mul_mod3_ne_zero (by omega) (by omega)
          have ha0 : ¬ (a % 2 = 0 ∨ a % 3 = 0) := by omega
          have hb0 : ¬ (b % 2 = 0 ∨ b % 3 = 0) := by omega
          have hab0 : ¬ ((a * b) % 2 = 0 ∨ (a * b) % 3 = 0) := by omega
          have hab : (a * b) % 12 = ((a % 12) * (b % 12)) % 12 := Nat.mul_mod a b 12
          have ha12 : a % 12 = 1 ∨ a % 12 = 5 ∨ a % 12 = 7 ∨ a % 12 = 11 := by
            have hlt : a % 12 < 12 := Nat.mod_lt a (by decide)
            have h212 : (a % 12) % 2 = a % 2 :=
              Nat.mod_mod_of_dvd a (show 2 ∣ 12 by decide)
            have h312 : (a % 12) % 3 = a % 3 :=
              Nat.mod_mod_of_dvd a (show 3 ∣ 12 by decide)
            rw [ha2'] at h212
            omega
          have hb12 : b % 12 = 1 ∨ b % 12 = 5 ∨ b % 12 = 7 ∨ b % 12 = 11 := by
            have hlt : b % 12 < 12 := Nat.mod_lt b (by decide)
            have h212 : (b % 12) % 2 = b % 2 :=
              Nat.mod_mod_of_dvd b (show 2 ∣ 12 by decide)
            have h312 : (b % 12) % 3 = b % 3 :=
              Nat.mod_mod_of_dvd b (show 3 ∣ 12 by decide)
            rw [hb2'] at h212
            omega
          rcases ha12 with ha1 | ha5 | ha7 | ha11
            <;> rcases hb12 with hb1 | hb5 | hb7 | hb11
          · have : (a * b) % 12 = 1 := by rw [hab, ha1, hb1]
            simp [chi3mod12, ha0, hb0, hab0, ha1, hb1, this]
          · have : (a * b) % 12 = 5 := by rw [hab, ha1, hb5]
            simp [chi3mod12, ha0, hb0, hab0, ha1, hb5, this]
          · have : (a * b) % 12 = 7 := by rw [hab, ha1, hb7]
            simp [chi3mod12, ha0, hb0, hab0, ha1, hb7, this]
          · have : (a * b) % 12 = 11 := by rw [hab, ha1, hb11]
            simp [chi3mod12, ha0, hb0, hab0, ha1, hb11, this]
          · have : (a * b) % 12 = 5 := by rw [hab, ha5, hb1]
            simp [chi3mod12, ha0, hb0, hab0, ha5, hb1, this]
          · have : (a * b) % 12 = 1 := by rw [hab, ha5, hb5]
            simp [chi3mod12, ha0, hb0, hab0, ha5, hb5, this]
          · have : (a * b) % 12 = 11 := by rw [hab, ha5, hb7]
            simp [chi3mod12, ha0, hb0, hab0, ha5, hb7, this]
          · have : (a * b) % 12 = 7 := by rw [hab, ha5, hb11]
            simp [chi3mod12, ha0, hb0, hab0, ha5, hb11, this]
          · have : (a * b) % 12 = 7 := by rw [hab, ha7, hb1]
            simp [chi3mod12, ha0, hb0, hab0, ha7, hb1, this]
          · have : (a * b) % 12 = 11 := by rw [hab, ha7, hb5]
            simp [chi3mod12, ha0, hb0, hab0, ha7, hb5, this]
          · have : (a * b) % 12 = 1 := by rw [hab, ha7, hb7]
            simp [chi3mod12, ha0, hb0, hab0, ha7, hb7, this]
          · have : (a * b) % 12 = 5 := by rw [hab, ha7, hb11]
            simp [chi3mod12, ha0, hb0, hab0, ha7, hb11, this]
          · have : (a * b) % 12 = 11 := by rw [hab, ha11, hb1]
            simp [chi3mod12, ha0, hb0, hab0, ha11, hb1, this]
          · have : (a * b) % 12 = 7 := by rw [hab, ha11, hb5]
            simp [chi3mod12, ha0, hb0, hab0, ha11, hb5, this]
          · have : (a * b) % 12 = 5 := by rw [hab, ha11, hb7]
            simp [chi3mod12, ha0, hb0, hab0, ha11, hb7, this]
          · have : (a * b) % 12 = 1 := by rw [hab, ha11, hb11]
            simp [chi3mod12, ha0, hb0, hab0, ha11, hb11, this]

theorem chi3mod12_add_twelve (n : Nat) : chi3mod12 (n + 12) = chi3mod12 n := by
  simp [chi3mod12, Nat.add_mod]

theorem chi4_add_twelve (n : Nat) : chi4 (n + 12) = chi4 n := by
  simp [chi4, Nat.add_mod]

def chi4chi3 (n : Nat) : Int := chi4 n * chi3mod12 n

theorem chi4chi3_mul (a b : Nat) :
    chi4chi3 (a * b) = chi4chi3 a * chi4chi3 b := by
  unfold chi4chi3
  rw [chi4_mul, chi3mod12_mul]
  calc
    chi4 a * chi4 b * (chi3mod12 a * chi3mod12 b)
        = chi4 a * (chi4 b * (chi3mod12 a * chi3mod12 b)) := by rw [Int.mul_assoc]
    _ = chi4 a * (chi3mod12 a * (chi4 b * chi3mod12 b)) := by
        rw [← Int.mul_assoc (chi4 b), Int.mul_comm (chi4 b), Int.mul_assoc]
    _ = (chi4 a * chi3mod12 a) * (chi4 b * chi3mod12 b) := by rw [← Int.mul_assoc]

theorem chi4chi3_add_twelve (n : Nat) : chi4chi3 (n + 12) = chi4chi3 n := by
  simp [chi4chi3, chi4_add_twelve, chi3mod12_add_twelve]

def oddRealChar_twelve_chi4 : OddRealChar 12 where
  χ := chi4
  map_mul := chi4_mul
  map_add_mod := chi4_add_twelve
  odd := fun _ => by decide

def oddRealChar_twelve_chi4chi3 : OddRealChar 12 where
  χ := chi4chi3
  map_mul := chi4chi3_mul
  map_add_mod := chi4chi3_add_twelve
  odd := fun _ => by decide

theorem chi4_eq_one_mod12 {q : Nat} (h : Nat.gcd q 12 = 1) :
    chi4 q = 1 ↔ q % 12 = 1 ∨ q % 12 = 5 := by
  have hodd := (gcd_twelve_eq_one h).1
  rcases coprime12_mod h with h1 | h5 | h7 | h11
  · constructor
    · intro; exact Or.inl h1
    · intro
      have h4 : q % 4 = 1 := by omega
      simp [chi4, hodd, h4]
  · constructor
    · intro; exact Or.inr h5
    · intro
      have h4 : q % 4 = 1 := by omega
      simp [chi4, hodd, h4]
  · constructor
    · intro hχ
      ...
      simp [chi4, hodd, h4] at hχ
    · intro hq
      rcases hq with h1' | h5' <;> omega
  · constructor
    · intro hχ
      have h4 : q % 4 = 3 := by omega
      simp [chi4, hodd, h4] at hχ
...
theorem chi4chi3_eq_one_mod12 {q : Nat} (h : Nat.gcd q 12 = 1) :
    chi4chi3 q = 1 ↔ q % 12 = 1 ∨ q % 12 = 7 := by
  rcases coprime12_mod h with h1 | h5 | h7 | h11
  · constructor
    · intro; exact Or.inl h1
    · intro
      simp [chi4chi3, chi4_of_mod12_one h1, chi3mod12_one h1]
  · constructor
    · intro hχ
      simp [chi4chi3, chi4_of_mod12_five h5, chi3mod12_five h5] at hχ
    · intro hq
      rcases hq with h1' | h7' <;> omega
  · constructor
    · intro; exact Or.inr h7
    · intro
      simp [chi4chi3, chi4_of_mod12_seven h7, chi3mod12_seven h7]
  · constructor
    · intro hχ
      simp [chi4chi3, chi4_of_mod12_eleven h11, chi3mod12_eleven h11] at hχ
    · intro hq
      rcases hq with h1' | h7' <;> omega

theorem classRough_of_a3_chi4 {p : Nat}
    (hcert : ∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4 q = 1) :
    ClassRough p 3 :=
  classRough_of_certifies (by decide : 0 < 3) oddRealChar_twelve_chi4 fun q hq hN hg => by
    have : p + 4 * (3 * 3) = p + 36 := by simp
    exact hcert q hq (by simpa [this] using hN) (by simpa using hg)

theorem classRough_of_a3_chi4chi3 {p : Nat}
    (hcert : ∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4chi3 q = 1) :
    ClassRough p 3 :=
  classRough_of_certifies (by decide : 0 < 3) oddRealChar_twelve_chi4chi3
    fun q hq hN hg => by
    have : p + 4 * (3 * 3) = p + 36 := by simp
    exact hcert q hq (by simpa [this] using hN) (by simpa using hg)

theorem classRough_a3_of_certificates {p : Nat}
    (h : (∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4 q = 1) ∨
         (∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4chi3 q = 1)) :
    ClassRough p 3 := by
  rcases h with h | h
  · exact classRough_of_a3_chi4 h
  · exact classRough_of_a3_chi4chi3 h

/-- ClassRough at `a = 3` is the union of the two odd real characters mod 12.
    An `11 (mod 12)` divisor is a `11`-prime or a `5×7` product. -/
theorem classRough_a3_iff_certificates {p : Nat} :
    ClassRough p 3 ↔
      (∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4 q = 1) ∨
      (∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4chi3 q = 1) := by
  constructor
  · intro h
    by_cases h4 : ∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4 q = 1
    · exact Or.inl h4
    · right
      intro q hq hdvd hcop
      by_cases hχ : chi4chi3 q = 1
      · exact hχ
      · have hn17 : ¬ (q % 12 = 1 ∨ q % 12 = 7) := by
          intro h17
          have : chi4chi3 q = 1 := (chi4chi3_eq_one_mod12 hcop).mpr h17
          exact hχ this
        have h511 : q % 12 = 5 ∨ q % 12 = 11 := by
          rcases coprime12_mod hcop with h1 | h5 | h7 | h11
          · exact False.elim (hn17 (Or.inl h1))
          · exact Or.inl h5
          · exact False.elim (hn17 (Or.inr h7))
          · exact Or.inr h11
        rcases h511 with h5 | h11
        · obtain ⟨r, hr⟩ := Classical.not_forall.mp h4
          obtain ⟨hrp, hr⟩ := Classical.not_imp.mp hr
          obtain ⟨hrd, hr⟩ := Classical.not_imp.mp hr
          obtain ⟨hrc, hrχ⟩ := Classical.not_imp.mp hr
          have hn15 : ¬ (r % 12 = 1 ∨ r % 12 = 5) := by
            intro h15
            have : chi4 r = 1 := (chi4_eq_one_mod12 hrc).mpr h15
            exact hrχ this
          have hr711 : r % 12 = 7 ∨ r % 12 = 11 := by
            rcases coprime12_mod hrc with h1 | h5' | h7 | h11
            · exact False.elim (hn15 (Or.inl h1))
            · exact False.elim (hn15 (Or.inr h5'))
            · exact Or.inl h7
            · exact Or.inr h11
          rcases hr711 with hr7 | hr11
          · have hne : r ≠ q := by intro heq; subst heq; omega
            have hprod : r * q ∣ p + 36 :=
              mul_dvd_of_distinct_primes hrp hq hne hrd hdvd
            have hmod : (r * q + 1) % 12 = 0 := by
              have : (r * q) % 12 = 11 := by
                have := Nat.mul_mod r q 12
                rw [hr7, h5] at this
                exact this
              omega
            have h3s : 3 ≤ r * q := by
        ...
        · have h3s : 3 ≤ q := by have := hq.1; omega
          exact absurd hdvd (h q h3s (by omega))
  · exact classRough_a3_of_certificates

/-- T(3) kernel: ClassRough on `a = 1,2,3` is exactly a finite union of
    character-kernel certificates (1×2×2 assignments). -/
theorem classRough_123_iff_certificates {p : Nat} :
    (ClassRough p 1 ∧ ClassRough p 2 ∧ ClassRough p 3) ↔
      (∀ q, IsPrime q → q ∣ p + 4 → Nat.gcd q 4 = 1 → chi4 q = 1) ∧
      ((∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4 q = 1) ∨
        (∀ q, IsPrime q → q ∣ p + 16 → Nat.gcd q 8 = 1 → chi4chi8 q = 1)) ∧
      ((∀ q, IsPrime q → q ∣ p + 36 → Nat.gcd q 12 = 1 → chi4 q = 1) ∨
  ...
  constructor
  · intro ⟨h1, h2, h3⟩
    exact ⟨classRough_iff_chi4.mp h1,
      classRough_a2_iff_certificates.mp h2,
      classRough_a3_iff_certificates.mp h3⟩
  · intro ⟨h1, h2, h3⟩
    exact ⟨classRough_iff_chi4.mpr h1,
      classRough_a2_iff_certificates.mpr h2,
      classRough_a3_iff_certificates.mpr h3⟩
